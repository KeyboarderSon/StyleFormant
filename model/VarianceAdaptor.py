...
# 우선 modules에 명시...
# by Kevin
# VarianceAdaptor, Length Regulator, VarianceEmbedding
class VarianceAdaptor(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Duration
        self.duration_predictor = VariancePredictor(self.hidden_dim, self.hidden_dim, self.predictor_kernel_size, dropout=self.dropout)
        # Pitch
        self.pitch_predictor = VariancePredictor(self.hidden_dim, self.hidden_dim, self.predictor_kernel_size, dropout=self.dropout)
        self.pitch_embedding = VarianceEmbedding(1, self.hidden_dim, self.embedding_kernel_size, self.dropout)
        # Energy
        self.energy_predictor = VariancePredictor(self.hidden_dim, self.hidden_dim, self.predictor_kernel_size, dropout=self.dropout)
