...
def get_audio(preprocess_config, wav_path):
    ...
    with open(
        ...
        pitch_mean, pitch_std = stats#, energy_mean, energy_std = stats
...
def synthesize(model, step, configs, vocoder, batchs, control_values):
    ...
    pitch_control, duration_control = control_values#energy_control, 

    for batch in batchs:
        batch = to_device(batch, device)
        with torch.no_grad():
            # Forward
            #print("batch[2:-1] is, ", batch[2:-1])
            output = model(
                *(batch[2:-1]),#FPF : batch[2:]
...
if __name__ == "__main__":
    ...
    if args.mode == "single":
        ...
        #     raw_texts, None, texts, text_lens, max(text_lens), [ref_info])]
        # batchs = [(ids, raw_texts, speakers, texts, text_lens, max(text_lens))]
    control_values = args.pitch_control, args.duration_control#args.energy_control, args.duration_control
